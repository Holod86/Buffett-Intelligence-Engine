...
with tab_scanner:
    ...
    if not df_display.empty:
        # Safe subset: only apply styling to columns that exist
        signal_cols = [c for c in ['Signal 1H', 'Signal 4H', 'Signal 1D'] if c in df_display.columns]
...
with tab_charts:
    if df_display.empty:
        ...
        with col_asset:
            selected_asset = st.selectbox("Актив", df_display["Asset"].tolist())
        with col_tf:
            selected_tf = st.selectbox(
                "Таймфрейм", ["1h", "4h", "1d"], index=2,
                format_func=lambda x: {"1h": "1 Час", "4h": "4 Часа", "1d": "1 День"}[x]
            )
        with col_zin:
            st.markdown("<br>", unsafe_allow_html=True)
            if st.button("🔍+", help="Увеличить (меньше свечей)", use_container_width=True):
                st.session_state.zoom_level = max(20, st.session_state.zoom_level - 30)
        with col_zout:
            st.markdown("<br>", unsafe_allow_html=True)
            if st.button("🔍−", help="Уменьшить (больше свечей)", use_container_width=True):
...
with tab_portfolio:
    ...
    all_assets = df_scan["Asset"].tolist() if not df_scan.empty else []
    
    with col_t1:
        trade_asset = st.selectbox("Актив", all_assets, key="trade_asset")
    with col_t2:
        ...
        trade_price = st.number_input("Цена ($)", value=float(current_price), min_value=0.01, step=0.01, key="trade_price")
    with col_t3:
        ...
        with col_buy:
            if st.button("🟢 BUY", use_container_width=True, key="btn_buy"):
                ...
                if success:
                    ...
                    st.toast(f"❌ {msg}")
        with col_sell:
            if st.button("🔴 SELL", use_container_width=True, key="btn_sell"):
